...
- **String-form alias targets are never split on `/`** (`resolver.ts:225-243`, `:316`): a string is a model id. Object form `{provider, model}` projects into the route field-by-field (`resolver.ts:235-243`) — extra metadata keys on the alias object survive schema validation (schemastery objects are pass-through — verified 2026-09-24). *Original contract (PR-A):* `protocol` was **not** projected into the resolved route and had to be read from the merged alias map (§4.3). *Superseded by #151 (user decision: misuse of gauge as a chat model is a hard error, not a warning):* a System One target — explicit `protocol: 'systemone'`, or a model id that contains the `llmbox_systemone/` segment — now **keeps `protocol: 'systemone'` on the resolved route** (`resolve`, `resolveDetailed`, `inspect`, incl. `$level` and one-hop follows); the chat paths reject such a route with `SystemOneChatModelError` (code `SYSTEMONE_NOT_CHAT_MODEL`) at the chat projections (`toAgentOptions` / `toOneShotRoute`) and at the `cc-model-routes` `llm/stream` guard. The marker is never forwarded into spawn `agentOptions` or one-shot `{provider, model}` pairs.
...
- Probes gauge via the **inspector** face (`ccModelRoutes.inspect`) for configured-ness — warning-free by construction (`resolver.ts:211-288`); when the service is unmounted, mirrors the overlay fallback (`service.ts:143-155`) over `createModelInspector` (both exported, `index.ts:16`). The inspector answers **only** configured-ness (`via` ∈ {`configured`, `one-hop`}); the protocol bit is read from the merged alias map entry via `mergeAliasMaps` (§4.3) through the shared `isSystemOneTarget` helper exported by `@dsh-cc/model-aliases`, with string-form entries excluded. Since #151 inspector/resolver routes do carry `protocol: 'systemone'` (§2), but arming deliberately keeps reading the entry: a string-form `llmbox_systemone/…` entry is marked System One on the route (so the chat guard rejects it) yet never arms. Armed ⇔ configured AND the object-form entry is System One (explicit `protocol: 'systemone'`, else a model id containing `llmbox_systemone/`).
...
- **Wrong-protocol footgun:** gauge resolved into a chat stream — since #151 this is a hard error: System One routes keep `protocol: 'systemone'`, the chat projections and the `llm/stream` guard throw `SystemOneChatModelError`, and tests fail if the stream adapter is invoked for gauge.
...
- **#151 (2026-09-26, contract change)** — per the user's decision that using gauge as a chat model is a hard error rather than a warning, `protocol` is no longer dropped by the resolver: System One targets keep `protocol: 'systemone'` on the resolved route, and chat paths (the `toAgentOptions` / `toOneShotRoute` projections and the `cc-model-routes` `llm/stream` guard) reject them with `SystemOneChatModelError` (`SYSTEMONE_NOT_CHAT_MODEL`). Route-policy arming semantics are unchanged (merged object-form entry; strings never arm). Same PR: bounded System One 429 retry and a half-open classifier breaker.
