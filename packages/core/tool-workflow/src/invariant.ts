/**
 * Package-owned durable workflow-record invariants for `@dsh-cc/tool-workflow`.
 * The cc preset mounts no invariant rows (the base-bundle posture), so this
 * companion has no claimant today; it is exported for profiles that opt into
 * invariant rows. Mirrors the harness `tool-workflow/src/invariant.ts` fold,
 * tolerating the dsh-cc `source` extension field on `run-start`.
 * @module @dsh-cc/tool-workflow/invariant
 */
...
const PACKAGE_NAME = '@dsh-cc/tool-workflow'
...
function cloneTraceForEvent(source: WorkflowTrace, event: SessionEvent, fail: InvariantFailure): WorkflowTrace {
  ...
  switch (event.type) {
    case 'tool-workflow/run-start': {
      ...
      if (data.source !== undefined
        && data.source !== 'inline' && data.source !== 'project-saved'
        && data.source !== 'user-saved' && data.source !== 'scriptPath') {
        fail('tool-workflow/run-start source must be inline/project-saved/user-saved/scriptPath when present')
      }
