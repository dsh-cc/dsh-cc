 ...
 * the core slice's disagreement rule). Returns the per-command disposers for
 * the caller to fold into its effect-scoped teardown.
 *
...
export function mountSavedWorkflowCommands(ctx: WorkflowCommandsContext, cwd: string): (() => void)[] {
  const commands = ctx.get('commands') as CommandsSeamLike | undefined
  ...
  for (const dir of savedWorkflowDirs(cwd)) {
    ...
    try {
      ...
      continue // absent directory: nothing saved there
    }
    for (const entry of entries) {
      ...
      if (!COMMAND_NAME.test(name)) {
        ctx.logger.warn(`tool-workflow: saved workflow "${path}" skipped — file name does not match the command-name rule ${COMMAND_NAME} (rename the file to mount it)`)
        continue
      }
      let script: string
      try {
        ...
        ctx.logger.warn(`tool-workflow: saved workflow "${path}" skipped — unreadable: ${String(error)}`)
        continue
      }
      const parsed = extractInlineMeta(script)
      if (!parsed.ok) {
        ctx.logger.warn(`tool-workflow: saved workflow "${path}" skipped — invalid meta (parse check): ${parsed.error}`)
        continue
      }
      const violations = metaShapeViolations(parsed.meta)
      if (violations.length > 0) {
        ctx.logger.warn(`tool-workflow: saved workflow "${path}" skipped — invalid meta (shape check): ${violations.join('; ')}`)
        continue
      }
      ...
      const definition = helpable(
        { name, description: meta.description, handler: run as never },
      ...
      try {
        ...
        // the already-registered command is the winner; skip-warn this one and
        // never abort the remaining mounts (plugin-loader precedent).
