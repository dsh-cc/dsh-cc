...
import { appendHookInvoked, appendHookResult, summarizeStderr, type HookOutput } from '@dsh-cc/hook-protocol'
...
describe('hook/* session events', () => {
  ...
  it('timedOut/parseFailure are recorded ONLY when true (absent otherwise)', () => {
    ...
    const results = [...session.snapshotEvents()].filter(e => e.type === 'hook/result')
