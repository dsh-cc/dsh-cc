# @dsh-cc/hooks-claude-code

English | [中文](README.zh.md)

A cordis plugin that runs the supported command-hook subset of a user's existing **Claude Code** hook config (a `hooks.json`, or a settings file's `hooks` key) on the harness's canonical interception points. It is the **CC dialect** half of the hooks subsystem: it owns the bridge's CC-shaped per-event stdin payloads, CC's env + `${CLAUDE_PLUGIN_ROOT}`/`${CLAUDE_PROJECT_DIR}` substitution, and the mapping from a hook's neutral outcome onto the harness's typed Decisions. The dialect-agnostic primitives (matcher, exit-code/stdout codec, `ctx.shell` execution, most-restrictive merge, the `hook/*` events) come from [`@dsh-cc/hook-protocol`](../hook-protocol/README.md).
...
```ts
import type { Config } from '@dsh-cc/hooks-claude-code'
const config: Config = {
...
Every agent-scoped stdin payload carries `session_id` and string-shaped `transcript_path`. The `transcript_path` is **always `''`**: the pinned persistence seam exposes no public artifact-path accessor (its jsonl backend's `locate` is private), so the bridge cannot resolve the path — the same behavior as upstream's own hooks-claude-code at the pin. (Even a resolved path would not create or flush the artifact, so it could be absent before the first turn-end checkpoint anyway.)

### Caller identity on payloads (`agent_id` / `agent_type`)

When a hook payload is built for a calling agent that is **currently a live in-process subagent**, the payload also carries `agent_id` (equal to the `SubagentStart` payload's id for the same child, grandchildren included) and a constant `agent_type: 'general-purpose'` — kind-specific real types stay a recorded parity gap. Membership comes from a **live set** keyed on the `subagent/start` / `subagent/end` events (added at start, removed at end), which covers the `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PermissionRequest`, `UserPromptSubmit`, `Stop`, `StopFailure`, and `TeammateIdle` payloads; non-subagent payloads gain nothing and stay byte-identical. Resume behavior: a child that **ended** and is then resumed as a top-level session — in the same process or a fresh one — emits **no** identity fields (its id left the live set at end, or was never in it); a **cold-resumed** continuable child re-enters the set because re-activation re-emits `subagent/start` before its next tool call. `SubagentStart`/`SubagentStop` payloads already carried both fields and are unchanged here. Note the session_id semantics difference from CC: dsh-cc's base reports the calling agent's own session id, so on a subagent payload `agent_id` equals `session_id` (CC reports the parent session id).
...
- **Supported hook events (20):** `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `Stop`, `SubagentStart`, `SubagentStop`, `PermissionRequest`, `PermissionDenied`, `Notification` (partial — `permission_prompt` subtype only), `PostCompact`, `SessionEnd`, `StopFailure`, `TaskCreated`, `TeammateIdle`, `Setup` (partial — first-run approximation), `SessionResume` (partial — `resume` source only), `WorktreeCreate`, and `WorktreeRemove` (WS-6: fired through the bridge's `hookRun` invoke seam by the worktree tools, subagent isolation, and TUI `/quit` cleanup; a `WorktreeCreate` hook exiting 0 with a stdout path replaces default creation, and a failing `WorktreeRemove` hook keeps the tree). **Unsupported (12):** `PreCompact` (needs an upstream `compaction/start` seam), `InstructionsLoaded`, `UserPromptExpansion`, `MessageDisplay`, `PostToolBatch`, `TaskCompleted`, `ConfigChange`, `CwdChanged`, `FileChanged`, `Elicitation`, `ElicitationResult`, and `UserPromptCancel` (dsh has no cancel seam — the bridge does not do a lossy approximation) — plus the `Notification` idle / `auth_success` / `elicitation` subtypes (headless cannot map them) and `SessionResume`'s `clear`/`compact` sources (no dsh emit point). Config for unsupported events is ignored before group parsing, so an unsupported event cannot invalidate or register hooks. The comparison baseline is Claude Code's [official hook-event reference](https://code.claude.com/docs/en/hooks#hook-events).
...
- **Error recovery is built in:** a turn that hits the model's output-token ceiling (the last assistant attempt's `max-tokens` finish frame) continues automatically as a next step in the same turn, up to 3 times (`CLAUDE_CODE_OUTPUT_TOKEN_CONTINUATION_CAP` overrides; `0` disables; invalid falls back to 3), steering Claude Code's resume wording *"Output token limit hit. Resume directly — no apology, no recap. Pick up mid-thought."* — the Stop hook is not invoked for the ceiling-hit stopping itself, and a normal recovered completion runs Stop as usual. Repeated API errors surface a durable notice at 3 consecutive (`CLAUDE_CODE_AGENT_ERROR_CONSECUTIVE_CAP`) or 20 cumulative (`CLAUDE_CODE_AGENT_ERROR_TOTAL_CAP`) errors — a warn plus a dim notice row naming the last classified error code; request retry stays harness-owned. Two documented divergences from Claude Code: (1) **Stop is suppressed during output-ceiling recovery** — a user-configured blocking Stop hook loses its veto window for the stopping event that immediately follows a ceiling hit (CC never runs Stop mid-recovery); Stop runs normally once the resumed step completes or the continuation cap is reached. (2) **The continuation is a model-visible next-step meta message** — CC continues the same logical response invisibly; here the resume wording is a plugin-sourced user-role next-step message (one extra model step per continuation, and the transcript still records one `turn/end` carrying the sticky `max-tokens` reason). The streak counters are also per-agent (a failing subagent child counts independently) where CC's caps are session-wide.
